var machinesPerformanceDirective = function () {
    ...
    const percentageKpis = ["UnitsRatio", "AvailabilityPE", "QualityIndex", "CycleTimeEfficiency", "PE", "CavitiesEfficiency", "PercentageByNumberOfEvents", "PercentageByDurationOfEvents"];
    ...
    const twoDigitsNumbers = ["Units/Hour Actual","Units/Hour std","Units/Min Actual","Units/Min std","UnitsProducedOK", "RejectsTotal", "UnitsProducedTheoretically", "TotalUnitsJosh","UnitsReportedOK"];
    const defaultCircleDisplayOrder = ["AvailabilityPE", "CycleTimeEfficiency"];
    // const defaultCircleDisplayOrder = ["PercentageByDurationOfEvents", "AvailabilityPE", "CycleTimeEfficiency", "PE"];
    ...
    // const defaultDisplayedTableColumns = ["UnitsRatio", "UnitsProducedOK", "RejectsTotal", "UnitsProducedTheoretically", "TotalUnitsJosh", "QualityIndex", "PE", "AvailabilityPE", "ProductionTime", "ActiveTime", "DownTime", "InActiveTime", "NumberOfSetups", "SetupDuration", "CycleTimeEfficiency", "CavitiesEfficiency", "PercentageByNumberOfEvents", "NumberOfEvents", "NumberOfReportedEvents", "PercentageByDurationOfEvents", "ReportedEventsDuration", "EventsDuration", "JoshID", "JobID", "Notes", "ERPJobID", "ProductName", "ShiftName","CatalogID","InActiveTime","JoshStartTime","JoshEndTime","Notes","ERPJobID","ProductName","ShiftName","WeightOfRejects"];
    ...
    var Template = 'views/custom/productionFloor/shiftTab/machinesPerformance/machinesPerformance.html';
    ...
    function controller($scope, shiftService, $timeout, $filter, LeaderMESservice, $localStorage, $rootScope, $state,machinesPerformanceService,$q) {
        $scope.loading = true;
        $scope.options.isActivityGaugeView = true;
        ...
        $scope.$on("machinesPerformanceUpdateHighcharts", (event, data) => {
            ...
            $scope.setStorage(ACTIVITY_GAUGE_FILTER_KEY, data.availableKpis)
            drawActivityGauge();
        ...
        $scope.updateDropdown = () => {
            if ($scope.initialized) {
                ...
                $scope.dataArr.displaySettings.maxDisplayedKpis = MAX_DISPLAYED_KPIS_GAUGE_VIEW;
        ...
        const computeOuterSeriesesArrayWorker = function (workersPerformanceDataArr, colors, translations, dataArrays) {
            for (let i = 0; i < workersPerformanceDataArr.length; i++) {
                ...
                for (let j = 0; j < colors.length; j++) {
                    ...
                    if (workersPerformanceDataArr[i].Active === 0 || workersPerformanceDataArr[i].WorkerName === "other") {
        ...
        const addRow = (tableData, data, joshRef) => {
            ...
            for (let j = 0; j < $scope.keys.length; j++) {
                ...
                if (!data.UnitsProducedTheoretically) { //rows of empty data of machines and other workers  
                    if ($scope.options.displayTableBy === 'worker') {
        ...
        const getKpiValue = (machineData, kpi) => {
            ...
            if (!val || !machineData.UnitsProducedTheoretically) {
        ...
        $scope.moveToActivityGaugeView = () => {
            $scope.availableKpis = $scope.getActivityGaugeKpis();
            $scope.updateDropdown();
            drawActivityGauge();
        }
        ...
        const createNewCircleDisplayListener = () => {
            $scope.CircleDisplayListener = $scope.$watch("options.displayCircleBy", function (newValue, oldValue) {
                if (newValue == oldValue) {
                    return;
                }
                if ($scope.options.displayCircleBy === null) {
                    $scope.options.displayCircleBy = 'machine';
                }
                drawActivityGauge();
            });
        }
        createNewCircleDisplayListener();

        let activityGaugeListener = $scope.$watch("options.isActivityGaugeView", function (newValue, oldValue) {
            if (JSON.stringify(newValue) == JSON.stringify(oldValue)) {
                return;
            }    
            $scope.moveToActivityGaugeView();        
        });
        $scope.addListener(activityGaugeListener);
            ...
            if ($scope.graph.dataArr) {
                if (!$scope.dataArr) {
                    ...
                    if ($scope.graph.dataArr.displaySettings) {
                        ...
                        $scope.setStorage(ACTIVITY_GAUGE_FILTER_KEY, $scope.graph.dataArr.displaySettings.availableKpis)
                        drawActivityGauge();                     
...
angular
    .module('LeaderMESfe')
    .directive('machinesPerformanceDirective', machinesPerformanceDirective);