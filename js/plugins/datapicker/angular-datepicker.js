...
//Moment format filter.
    Module.filter('mFormat', function () {
        return function (m, format, tz, custom) {
            let customFilter = null;
            if (custom) {
                customFilter = sessionStorage.getItem('userDateFormat');
            }
            if (!m) {
                return undefined;
            }
            if (!(moment.isMoment(m))) {
                return moment(m).format(customFilter || format);
            }
            if (tz){
                return moment.tz(m, tz).format(customFilter || format);
            }
            else{
                var momentDate = m.format(customFilter || format);
                return (momentDate == 'Invalid date' ? undefined : momentDate);
            }
        };
    });
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                scope.setView = function (nextView) {
                    if (scope.views.indexOf(nextView) !== -1) {
                        ...
                        if (typeof scope.model === 'string'){
                            scope.model = moment(scope.model,format);
                        }
                    }
                };
                ...
                function update() {
                    ...
                    if (scope.model && !arrowClick) {
                        if (typeof scope.model === "string" && scope.model != "" && scope.model != null) {
                            scope.date = moment(scope.model, attrs.format);
                        }
                        else{
    ...
    Module.directive('dateTime', ['$compile', '$document', '$filter', 'dateTimeConfig', '$parse', 'datePickerUtils', function ($compile, $document, $filter, dateTimeConfig, $parse, datePickerUtils) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                //     return element.attr('format'); 
                // }, function(newValue){
                //     format = newValue || dateTimeConfig.format;
                // });     
                ...
                function formatter(value) {
                    return dateFilter(value, format, timezone, customBoolen == "false" ? false : true);
                }
                ...
                function parser(viewValue) {
                    ...
                    else if (moment(new Date(viewValue))){
                        return viewValue;
                    }
                    return undefined;
                }
                ...
                function showPicker() {
                    ...
                    if (position === 'absolute') {
                        ...
                        body.append(picker);
                        var rectPageBottom = element[0].getBoundingClientRect().bottom;
                        var pageHeight = document.body.clientHeight;

                        if (picker.height() + rectPageBottom >= pageHeight) {
                            picker.css({ top: (pos.top + $('html').scrollTop() - picker.height() - 8) + 'px', left: pos.left + 'px', display: 'block', position: position });
                        }
                        else{
                            picker.css({ top: (pos.top + height + $('html').scrollTop()) + 'px', left: pos.left + 'px', display: 'block', position: position });
                        }
                        ...
                        var pageHeight = document.body.clientHeight;
                        if (picker.height() + rectPageBottom >= pageHeight) {
                            picker.css({bottom: '0px', display: 'block'});
                        }
                        else{
