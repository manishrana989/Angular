 ...
 */
(function($, moment) {
	if (!$ || !moment) throw 'Jquery and moment lib must be loaded first';

	var available_options = ['minDate', 'maxDate', 'dateLimit', 'showDropdowns', 'showWeekNumbers', 'showISOWeekNumbers', 'timePicker', 'timePickerIncrement', 'timePicker24Hour', 'timePickerSeconds', 'ranges', 'showCustomRangeLabel', 'alwaysShowCalendars', 'opens', 'drops', 'buttonClasses', 'applyClass', 'cancelClass', 'locale', 'singleDatePicker', 'autoApply', 'linkedCalendars', 'isInvalidDate', 'isCustomDate', 'autoUpdateInput', 'parentEl'];

	angular.module('angularjs.daterangepicker', []);

	angular
		.module('angularjs.daterangepicker')
					...
					function openListener() {
						$scope.$watchGroup(watchList, function(n) {
							if (n && n.length > 0) {
								...
								if (minDate) dpApi.setMinDate(minDate);

								if (maxDate) dpApi.setMaxDate(maxDate);

								if (startDate) dpApi.setStartDate(startDate);

								if (endDate) dpApi.setEndDate(endDate);
							}
						});
					}
	...
	DateRangePickerApi.prototype.build = function() {
		...
		return {
			setMinDate: function(x) {
				...
				if (self.$scope.startDate && self.$scope.startDate.isBefore(x)) {
					self.$scope.startDate = x;
				}

				if (self.$scope.endDate && self.$scope.endDate.isBefore(x)) {
			...
			setMaxDate: function(x) {
				...
				if (self.$scope.startDate && self.$scope.startDate.isAfter(x)) {
					self.$scope.startDate = null;
				}

				if (self.$scope.endDate && self.$scope.endDate.isAfter(x)) {
	...
	DateRangePickerApi.prototype.onCancel = function(event, api) {
		...
		if (self.$scope.clearOnCancel) {
			self.$scope.$apply(function() {
				self.$scope.startDate = null;
				self.$scope.endDate = null;
				if (self.$scope.formCtrl && self.$scope.ngRequired) self.$scope.formCtrl.$setValidity(self.$scope.dateRangePickerId, false);

				self.clearInput();
				try {
					if (self.$scope.onChange) self.$scope.onChange(api);
				} catch (e) {
					throw e;
				}
			});
	...
	DateRangePickerApi.prototype.onHide = function(event, api) {
		...
		setInterval(function() {
			if (self.isSingle()) {
				if (!self.$scope.startDate) self.clearInput();
			} else {
				if (!self.$scope.startDate && !self.$scope.endDate) self.clearInput();
			}
		});
	};
